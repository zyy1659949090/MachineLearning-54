...
The penalized negative log-likelihood is defined as:
    ...
    −[(1/N)*sum(sum(y[i, k]*(t(x[i]* βk + βk0, k = 1:K), i = 1:N) − log(sum(exp(t(x[i]*βk + βk0, k = 1:K)] + λ*[((1 − α)/2)*(||β||F)^2 + α*sum(||βj||1, j = 1:P)]

where βc is vector of coefficients for class c and y[i, k] is kth element of the binary vector produced by expanding the response variable using one-hot encoding (i.e. y[i, k] == 1 iff the response at the ith observation is k. It is 0 otherwise.
...
Tweedie distributions are a family of distributions which include gamma, normal, Poisson and their combination. It is especially useful for modeling positive continuous variables with exact zeros. The variance of the Tweedie distribution is proportional to the p-th power of the mean var(y[i]) = φ*μ[i]^p.
...
The corresponding deviance when p != 1 and p != 2 is equal to:
    ...
    D = −2*sum(y[i]*(y[i]^(1 − p) − yˆ[i]^(1 − p)) − (y[i]^(2 − p) − yˆ[i]^(2 − p))/(2 − p), i = 1:N)
...
- lambda\_min\_ratio and nlambdas: The sequence of λs is automatically generated as an exponentially decreasing sequence. It ranges from λmax, the smallest λ so that the solution is a model with all 0s, to λmin = lambda\_min\_ratio*λmax.
...
When using the l1 penalty with lambda search, specify a value for the max\_active\_predictors parameter to stop the search before it completes.
...
You can request p-values by setting the compute\_p\_values option. It can only be used with the IRLSM solver and no regularization. It is recommended that you also set the remove\_collinear\_columns option. Otherwise, H2O will return an error if it detects collinearity in the dataset and p-values are requested.
...
sorted(binomial_fit.coef_norm().items(), 
       key = lambda x:x[1], 
       reverse = True)
```
...
MSE is the mean squared error: 
    ...
    MSE = (1/N)*sum((actual[i] − prediction[i])^2, i - 1:N)

Rˆ2 is the R squared: 

    R^2 = 1 − MSE/σ[y]^2

LogLoss is the log loss. 

    LogLoss = (1/N)*sum(sum(y[i]*log(p[i, j]), j = 1:C), i = 1:N)

AUC is available only for binomial models and is defined the area under ROC curve.

Null_deviance: Deviance (defined by selected family) computed for the null model.

Residual_deviance: Deviance of the built model
...
binomial_fit._model_json["output"]["model_summary"].
...
# h2o.exportFile(pred, "hdfs://namenode/path/to/file
.csv")
...
for(i in 1:5) {
  ...
  print(paste("cv fold ", i, " training auc:",
     cv_model@model$training_metrics@metrics$AUC, 
     " validation auc: ", 
     cv_model@model$validation_metrics@metrics$AUC))
}
```
...
for model_ in model.get_xval_models():
    print model_.model_id, " training auc:",
       model_.auc(), " validation auc: ", model_.
       auc(valid=True)
```

- Look for explained deviance (1 − (NullDev − ResDev)/ NullDev)
    + Too close to 0: model doesn‘t predict well (under-fitting)
...
- For logistic regression (i.e. binomial classification) models, look for AUC
    + Too close to 0.5: model doesn‘t predict well (under-fitting)
    + Too close to 1: model predicts too well due to noisy data (overfitting)
...
Categorical variables have special handling during GLM computation as well. When forming the gram matrix, we can take advantage of the fact that columns belonging to the same categorical never co-occur and the gram matrix region belonging to these columns will not have any non-zero elements outside of the diagonal.

This keeps it in sparse representation, taking only O(N) elements instead of O(N*N). Furthermore, the complexity of Choelsky decomposition of a matrix that starts with a diagonal region can be greatly reduced. H2O’s GLM exploits these two facts to handle the largest categorical “for free”. Therefore, when analyzing the performance of GLM in the equation expressed above, we can subtract the size of the largest categorical variables from the number of predictors.

    N = sum(∥c.domain∥, c ∈ C) − arg max(∥c.domain∥, c ∈ C) + ∥Nums∥
...
2. Compute the weighted gram matrix t(X)\*W\*X and t(X)\*z vector
...
- Gram matrix (t(X)*X) (distributed)
    + CPU:  O(M\*N^2/(p\*n))
    + Memory:  O(training data) + O(gram matrix), O(M\*N) + O(N^2/(p\*n)
...
Its decomposition cost grows as the cube of the number of predictors increases, which is a computational cost issue. In many cases, H2O can work around these limitations due to its handling of categorical variables and by employing strong rules to filter out inactive predictors.
...
The cost of the coefficient update is k\*N, where N is number of predictors and k is a constant, the cost of gradient computation is M\*N/(p\*n) where M is number of observations in the dataset and p\*n is the number of CPU cores in the cluster. Since k is a small constant, the runtime of L-BFGS is dominated by the gradient computation, which is fully parallelized, scaling L-BFGS almost linearly.
...
validation frame: An H2OParsedData object containing the validation dataset used to construct confusion matrix. If blank, the training data is used by default.
...
tweedie\_variance\_power: A numeric specifying the power for the variance function when family = "tweedie". Default is 0.

tweedie\_link\_power: A numeric specifying the power for the link function when family = "tweedie". Default is 1.
...
fold_assignment: Cross-validation fold assignment scheme, if fold column is not specified. The following options are supported: AUTO, Random, or Modulo.
...
missing\_values\_handling: Handling of missing values. Either Skip or MeanImputation (default).
...
Stephen Boyd, Neal Parikh, Eric Chu, Borja Peleato, and Jonahtan Eckstein. Distributed Optimization and Statistical Learning via the Alternating Di- rection Method of Multipliers. Foundations and Trends in Machine Learning, 3(1):1–122, 1996. URL https://web.stanford.edu/~boyd/papers/pdf/admm_distr_stats.pdf
